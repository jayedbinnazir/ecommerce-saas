...
//	go run ./cmd/migration down 3         # roll back the last 3 migrations
...
func runUp(db *sql.DB, all []migration, applied map[int64]bool) {
	...
	for _, m := range all {
		...
		if err := exec(db, m.upSQL, `INSERT INTO schema_migrations (version, name) VALUES ($1, $2)`, m.version, m.name); err != nil {
...
func runDown(db *sql.DB, all []migration, applied map[int64]bool, steps int) {
	...
	for i := len(all) - 1; i >= 0 && ran < steps; i-- {
		...
		if err := exec(db, m.downSQL, `DELETE FROM schema_migrations WHERE version = $1`, m.version); err != nil {
...
// exec runs the migration body and its bookkeeping statement in one transaction.
