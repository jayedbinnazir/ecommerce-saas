...
const subColumns = `id, user_id, plan_id, status, current_period_start, current_period_end, created_at, updated_at`

func scanSub(row platform.Scanner) (*domain.Subscription, error) {
	var s domain.Subscription
	err := row.Scan(&s.ID, &s.UserID, &s.PlanID, &s.Status, &s.CurrentPeriodStart, &s.CurrentPeriodEnd, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &s, nil
...
func (r *SubscriptionRepository) Update(ctx context.Context, s *domain.Subscription) error {
	...
	updated, err := scanSub(r.db.QueryRowContext(ctx, q, s.ID, s.Status, s.CurrentPeriodStart, s.CurrentPeriodEnd))
	if errors.Is(err, sql.ErrNoRows) {
